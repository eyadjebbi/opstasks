    ...
    <div style={s.page}>
      ...
      {loading ? (
        ...
        <div style={s.emptyState}>
          ...
          <p style={{color:"#94a3b8", marginTop:"0.75rem", fontSize:"1.1rem"}}>Aucune tache. Creez-en une !</p>
        </div>
      ) : (
              ...
              {editingTask === task.id ? (
                ...
                <div style={s.taskInner}>
                  <div style={s.taskLeft}>
                    <div style={s.taskTopRow}>
                      <span style={{...s.badge, background: STATUS_COLORS[task.status]}}>
                        {task.status === "todo" ? "○" : task.status === "in_progress" ? "▶" : "✓"} {STATUS_LABELS[task.status]}
                      </span>
                  ...
                  <div style={s.taskActions}>
                    ...
                    <button style={s.btnEdit} onClick={() => startEdit(task)}>
                      ✏️ Modifier
                    </button>
                    <button style={s.btnDelete} onClick={() => deleteTask(task.id)}>
                      🗑️ Supprimer
                    </button>
                  </div>
                </div>
...
const s = {
  ...
  subtitle: { color: "#64748b", margin: "0.4rem 0 0 0", fontSize: "1.1rem" },
  ...
  cardTitle: { fontSize: "1.2rem", fontWeight: 600, color: "#334155", marginTop: 0, marginBottom: "1.25rem" },
  form: { display: "flex", gap: "0.75rem", flexWrap: "wrap" },
  input: { flex: 1, minWidth: 200, padding: "0.75rem 1rem", border: "1px solid #cbd5e1", borderRadius: 8, fontSize: "1rem", outline: "none" },
  btnAdd: { padding: "0.75rem 1.75rem", background: "#3b82f6", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 600, fontSize: "1rem", whiteSpace: "nowrap" },
  taskList: { display: "flex", flexDirection: "column", gap: "1rem" },
  taskCard: { background: "#fff", borderRadius: 10, padding: "1.5rem 1.75rem", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" },
  taskInner: { display: "flex", justifyContent: "space-between", alignItems: "center", gap: "1.5rem", flexWrap: "wrap" },
  taskLeft: { flex: 1 },
  taskTopRow: { display: "flex", alignItems: "center", gap: "1rem", marginBottom: "0.6rem" },
  badge: { color: "#fff", fontSize: "0.85rem", padding: "0.3rem 0.9rem", borderRadius: 999, fontWeight: 600 },
  taskDate: { fontSize: "0.85rem", color: "#94a3b8" },
  taskTitle: { fontWeight: 600, color: "#1e293b", margin: "0 0 0.35rem 0", fontSize: "1.1rem" },
  taskDesc: { color: "#64748b", fontSize: "0.95rem", margin: 0 },
  taskActions: { display: "flex", gap: "0.6rem", flexShrink: 0, flexWrap: "wrap" },
  btnAdvance: { padding: "0.55rem 1.1rem", background: "#dbeafe", color: "#1d4ed8", border: "1px solid #bfdbfe", borderRadius: 8, cursor: "pointer", fontSize: "0.95rem", fontWeight: 500 },
  btnEdit: { padding: "0.55rem 1.1rem", background: "#fef9c3", color: "#92400e", border: "1px solid #fde68a", borderRadius: 8, cursor: "pointer", fontSize: "0.95rem", fontWeight: 500 },
  btnDelete: { padding: "0.55rem 1.1rem", background: "#fee2e2", color: "#dc2626", border: "1px solid #fca5a5", borderRadius: 8, cursor: "pointer", fontSize: "0.95rem", fontWeight: 500 },
  btnSave: { padding: "0.6rem 1.25rem", background: "#10b981", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontSize: "1rem", fontWeight: 600 },
  btnCancel: { padding: "0.6rem 1.25rem", background: "#f1f5f9", color: "#475569", border: "1px solid #cbd5e1", borderRadius: 8, cursor: "pointer", fontSize: "1rem" },
  editForm: { display: "flex", flexDirection: "column", gap: "0.75rem" },
  actRow: { display: "flex", gap: "0.75rem" },
  errorBox: { background: "#fee2e2", color: "#dc2626", border: "1px solid #fca5a5", borderRadius: 10, padding: "1rem 1.5rem", marginBottom: "1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "1rem" },
  closeBtn: { background: "none", border: "none", cursor: "pointer", color: "#dc2626", fontWeight: 700, fontSize: "1.1rem" },
